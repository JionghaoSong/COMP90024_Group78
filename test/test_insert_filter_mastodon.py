...
''')
    ...
    def test_insert_melbourne_mastodon_social_data(self, mock_bulk, mock_es, mock_open):
        # Mock Elasticsearch client
        mock_client = MagicMock()
        mock_es.return_value = mock_client
        
        # Run the function
        insert_melbourne_mastodon_social_data('dummy_path.json', batch_size=2)
        ...
        for action in actions:
            ...
            self.assertEqual(action['_index'], 'melbourne_mastodon_social')
...
''')
    @patch('elasticsearch.Elasticsearch')
    @patch('elasticsearch.helpers.bulk')
    def test_insert_melbourne_mastodon_social_data_with_bulk_error(self, mock_bulk, mock_es, mock_open):
        # Mock Elasticsearch client
        mock_client = MagicMock()
        mock_es.return_value = mock_client
        ...
        with self.assertRaises(Exception):
            insert_melbourne_mastodon_social_data('dummy_path.json', batch_size=1)
